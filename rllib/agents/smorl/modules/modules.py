...
class ConvLSTMEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_lstm = ConvLSTM(input_dim=img_encode_dim, hidden_dim=[conv_lstm_hid_dim, ] * 2,
                                  kernel_size=(3, 3), num_layers=2, map_h=ARCH.NUM_CELL_H,
                                  map_w=ARCH.NUM_CELL_W)
    ...
    def forward(self, x, actions):
        ...
        img_enc, log_list = self.conv_lstm(img_conv_enc, actions)

        return img_enc[0], log_list

    def one_step(self, x, action):
        """

        :param x: (bs, dim, cell_h, cell_w)
        :return:
        """
        bs = x.size(0)
        img_conv_enc = self.image_enc(x)

        img_conv_enc = img_conv_enc.view(bs, img_conv_enc.size(-3), img_conv_enc.size(-2), img_conv_enc.size(-1))

        img_enc = self.conv_lstm.one_step(img_conv_enc, action)
...
class ConvLSTMCell(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, kernel_size=3, map_h=8, map_w=8):
        ...
        self.conv_h = nn.Conv2d(in_channels=self.hidden_dim,
                                ...
                                padding=self.padding,
                                bias=False)
        self.fc_a = nn.Linear(in_features=2, out_features= 4 * self.hidden_dim, bias=True)
    ...
    def forward(self, x, action, h_c):
        h_cur, c_cur = h_c
        xi, xf, xo, xc = self.conv_x(x).split(self.hidden_dim, dim=1)
        ai, af, ao, ac = self.fc_a(action).split(self.hidden_dim, dim=1)
        ai, af, ao, ac = ai[..., None,None], af[..., None,None], ao[..., None,None], ac[..., None,None]
        hi, hf, ho, hc = self.conv_h(h_cur).split(self.hidden_dim, dim=1)

        i = torch.sigmoid(xi + hi + ai + c_cur * self.Wci)
        f = torch.sigmoid(xf + hf + af + c_cur * self.Wcf)
        c_next = f * c_cur + i * torch.tanh(xc + hc + ac)
        o = torch.sigmoid(xo + ho + ao + c_cur * self.Wco)
...
class ConvLSTM(nn.Module):
        ...
        for layer_idx in range(self.num_layers):
            ...
            for t in range(seq_len):
                ...
                h, c, log = self.cell_list[layer_idx](x=cur_layer_input[:, t, :, :, :],
                                                      action=actions[:, t, :],
                                                    h_c=[self.hs[layer_idx], self.hs[layer_idx]])
                self.hs[layer_idx] = h
                self.cs[layer_idx] = c
    ...
    def one_step(self, x, action):
        ...
        cur_layer_input = x
        for layer_idx in range(self.num_layers):
            h, c, log = self.cell_list[layer_idx](x=cur_layer_input,
                                                    action=action,
                                                    h_c=[self.hs[layer_idx], self.hs[layer_idx]])
            self.hs[layer_idx] = h
            self.cs[layer_idx] = c
