...
class ProposalCore(nn.Module):
    ...
    def update_z_where(self, z_where):
        z_where_origin = z_where.clone()

        scale, ratio = z_where[:, :2].tanh().chunk(2, 1)
        scale = ARCH.SIZE_ANC + ARCH.VAR_S * scale  # add bias to let masking do its job
        ratio = ARCH.RATIO_ANC + ARCH.VAR_ANC * ratio
        ratio_sqrt = ratio.sqrt()
        z_where[:, 0:1] = scale / ratio_sqrt
        z_where[:, 1:2] = scale * ratio_sqrt
        z_where[:, 2:] = 2. / ARCH.NUM_CELL_H * (self.offset + 0.5 + z_where[:, 2:].tanh()) - 1
        # looks like z_where[:, 2:]  is an absolute pos from -1 to 1. 
        z_where = z_where.permute(0, 2, 3, 1).reshape(-1, 4) 
        return z_where, z_where_origin
    ...
    def forward(self, img_enc, alpha, tau, t, gen_pres_probs=None, gen_depth_mean=None,
        ...
        if ARCH.PHASE_GENERATE and t >= ARCH.OBSERVE_FRAMES:
            gen_pres_logits = probs_to_logits(gen_pres_probs, is_binary=True).view(1, 1, 1, 1). \
                expand(bs, -1, ARCH.NUM_CELL_H, ARCH.NUM_CELL_W)
            z_pres_logits = gen_pres_logits
            z_depth_mean, z_depth_std = gen_depth_mean.view(1, -1, 1, 1).expand(bs, -1, ARCH.NUM_CELL_H,
                                                                                ARCH.NUM_CELL_W), \
                                        gen_depth_std.view(1, -1, 1, 1).expand(bs, -1, ARCH.NUM_CELL_H,
                                                                               ARCH.NUM_CELL_W)
            z_where_mean, z_where_std = gen_where_mean.view(1, -1, 1, 1).expand(bs, -1, ARCH.NUM_CELL_H,
                                                                                ARCH.NUM_CELL_W), \
                                        gen_where_std.view(1, -1, 1, 1).expand(bs, -1, ARCH.NUM_CELL_H,
                                                                               ARCH.NUM_CELL_W)
        else:
        ...
        # (bs, dim, 8, 8)
        z_where, z_where_origin = self.update_z_where(z_where)
        z_where_mean, z_where_mean_origin = self.update_z_where(z_where_mean)

        return z_where, z_pres, z_depth, z_where_mean, z_where_std, \
               z_depth_mean, z_depth_std, z_pres_logits, z_pres_y, z_where_origin, z_where_mean_origin


class ProposalRejectionCell(nn.Module):

    def __init__(self, z_what_net, glimpse_dec_net):
        ...
        self.z_pres_anneal_end_value = ARCH.Z_PRES_ANNEAL_END_VALUE
        self.z_pres_masked_prior = 1e-8
        self.max_num_obj = ARCH.MAX_NUM_OBJ

        self.ProposalNet = ProposalCore()
        ...
        self.register_buffer('num_cell', torch.tensor(ARCH.NUM_CELL_H * ARCH.NUM_CELL_W))
    ...
    def forward(self, x, img_enc, alpha_map_prop, ids_prop, lengths, t, no_disc=False, eps=1e-15, 
        global_step=0):
        """
            # TODO (Chmin): should parse globals step.
        ...
        self.prior_z_pres_prob = linear_annealing(global_step, self.z_pres_anneal_start_step,
        ...
        z_depth_mean, z_depth_std, z_pres_logits, z_pres_y, z_where_origin, z_where_mean_origin = self.ProposalNet(
            img_enc, alpha_map_prop, ARCH.TAU, t, gen_pres_probs=x.new_ones(1) * ARCH.GEN_DISC_PRES_PROBS,
        ...
        if t > 0 and no_disc:
            z_pres = torch.zeros_like(z_pres)
        q_z_where = Normal(z_where_mean_origin, z_where_std)
        q_z_depth = Normal(z_depth_mean, z_depth_std)

        z_pres_orgin = z_pres

        if ARCH.PHASE_GENERATE and t >= ARCH.OBSERVE_FRAMES:
            z_what_mean, z_what_std = self.prior_what_mean.view(1, 1).expand(bs * ARCH.NUM_CELL_H *
                                                                             ARCH.NUM_CELL_W, z_what_dim), \
                                      self.prior_what_std.view(1, 1).expand(bs * ARCH.NUM_CELL_H *
                                                                            ARCH.NUM_CELL_W, z_what_dim)
            x_att = x.new_zeros(1)
        else:
        ...
        if phase_rejection and t > 0:
            ...
            pres_mask = (explained_ratio < ARCH.EXPLAINED_RATIO_THRESHOLD).view(bs, 1, num_cell_h, num_cell_w).float()
        ...
        if ARCH.LOG_PHASE:
            ...
            self.log = {}
        representation = {"z_where": z_where_mean.view(bs, num_cell_h * num_cell_w, -1), 
                          "z_what": z_what_mean.view(bs, num_cell_h * num_cell_w, -1), 
                          "z_depth": z_depth_mean.view(bs, num_cell_h * num_cell_w, -1)}
        return y_each_cell.view(bs, num_cell_h * num_cell_w, 3, img_h, img_w), \
               ...
               log_imp, self.log, representation
