...
class AttEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.temporal_img_enc_net = nn.Linear(
            temporal_img_enc_hid_dim * ARCH.NUM_CELL_H // 2 * ARCH.NUM_CELL_W // 2, temporal_img_enc_dim)
...
class PropagationCell(nn.Module):
    ...
    def __init__(self, z_what_net, glimpse_dec_net):
        ...
        self.attention_encoding = AttEncoder()
    ...
    def forward(self, x, img_enc, temporal_rnn_out_pre, temporal_rnn_hid_pre, prior_rnn_out_pre,
        ...
        img_enc_att = spatial_transform(
            img_enc.unsqueeze(1).expand(-1, max_num_obj, -1, -1, -1).contiguous().
                view(bns, img_encode_dim, ARCH.NUM_CELL_H, ARCH.NUM_CELL_W), z_where_att,
            (bns, img_encode_dim, ARCH.NUM_CELL_H // 2, ARCH.NUM_CELL_W // 2), inverse=False
        )
        # bns, dim
        ...
        if ARCH.PHASE_GENERATE and t >= ARCH.OBSERVE_FRAMES:
            q_z_pres = NumericalRelaxedBernoulli(probs=prior_pres_prob, temperature=ARCH.TAU)
        else:
            q_z_pres = NumericalRelaxedBernoulli(logits=z_pres_logits, temperature=ARCH.TAU)
